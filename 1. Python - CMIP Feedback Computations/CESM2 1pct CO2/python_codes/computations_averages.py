...
def import_var(col, model_name, var_name):
    ...
    # Get rid of unnecessary coordinates and split in 1pct and control datasets
    ...
    # Compute the monthly averages for control experiment and track changes in the 1pct experiment compared to those
...
def comp_moisture_change(col, model_name):
    # Obtain initial q1
    ...
    # Use the abrupt experiment for the scaling (better noise-to-signal ratio!)
    cat = col.search(
        source_id = model_name,
    ...
    # Compute the monthly averages for control experiment and track changes in the 1pct experiment compared to those
    ctrl_ta_monthly_av = ds_ta_ctrl['ta'].groupby('time.month').mean(dim='time')
    dta = ds_ta_abr['ta'].groupby('time.month') - ctrl_ta_monthly_av
