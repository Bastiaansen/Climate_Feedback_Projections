function save_projection_CLOUD(file_name, proj_var, error, var_minus_forcing, lon, lat, year)
...
%% DATA
nccreate(file_name, 'Cloud_Feedback_Data', 'dimensions', {'lon', length(lon), 'lat', length(lat), 'year', length(year)});
ncwrite(file_name, 'Cloud_Feedback_Data', var_minus_forcing)
